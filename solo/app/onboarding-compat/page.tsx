  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <span className="text-xs text-[#71717A] tabular-nums">{current + 1} / {total}</span>
        <span className="text-xs font-semibold text-[#8B5CF6] tabular-nums">{pct}%</span>
      </div>
    ...
    <button
      ...
      className={`w-full px-5 py-[1.05rem] rounded-2xl border text-left text-sm font-medium transition-all duration-150 flex items-center justify-between gap-3
        ${selected
          ...
          : "bg-[#FAFAFA] border-zinc-200 text-[#18181B] hover:border-zinc-300 hover:bg-zinc-100"
        }`}
    >
  ...
  return (
    <div className="flex flex-col gap-7">
      <div className="flex flex-col gap-2.5">
        ...
        <h1
          className="text-[1.6rem] font-black text-[#18181B] leading-tight"
  ...
  if (initError) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <div>
          <p className="text-zinc-900 font-semibold text-sm">Algo deu errado</p>
          <p className="text-zinc-500 text-xs mt-1 max-w-xs">{initError}</p>
        </div>
        <button
          onClick={() => { setInitError(""); setRetryCount((c) => c + 1); }}
          className="mt-2 px-5 py-2.5 bg-[#8B5CF6] hover:bg-violet-400 text-white text-sm font-semibold rounded-xl transition-colors"
        >
          Tentar novamente
        </button>
      </div>
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 pt-5 pb-4">
        <span
          className="text-xl font-black text-[#8B5CF6]"
        ...
        <Link
          href="/dashboard"
          className="text-xs text-[#71717A] hover:text-zinc-600 transition-colors"
        >
          Pular por agora
        </Link>
      </div>
      ...
      {/* Error */}
      {error && (
        <div className="mx-6 mb-4 px-4 py-3 rounded-2xl bg-red-50 border border-red-200 text-red-600 text-sm">
          {error}
        </div>
      )}
      ...
      <div className="px-6 pb-8 flex gap-3">
        {!isFirst && (
          <button
            onClick={goBack}
            disabled={saving}
            className="flex-1 py-4 rounded-2xl border border-zinc-200 text-[#18181B] font-semibold text-sm hover:bg-zinc-50 hover:border-zinc-300 transition-all duration-200 disabled:opacity-40"
          >
            Voltar
          </button>
        )}
        <button
          ...
          className="flex-1 py-4 rounded-2xl bg-[#8B5CF6] hover:bg-violet-400 active:bg-violet-600 disabled:bg-zinc-100 disabled:text-[#71717A] disabled:cursor-not-allowed text-white font-semibold text-sm transition-all duration-200 shadow-md shadow-violet-500/20 hover:-translate-y-px active:translate-y-0 flex items-center justify-center gap-2"
        >
          {saving && (
