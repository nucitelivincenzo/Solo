...
export default function CadastroPage() {
  ...
  if (emailSent) {
    return (
      <main className="min-h-screen bg-[#050506] flex items-center justify-center px-6">
        <div className="w-full max-w-sm text-center flex flex-col gap-6">
          ...
          <div>
            ...
            <p className="text-[#A1A1AA] text-sm mt-2 leading-relaxed">
              Enviamos um link para <span className="text-amber-400 font-medium">{form.email}</span>. Clique no link para ativar sua conta.
            </p>
          </div>
      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 mb-6 justify-center">
            <div className="relative w-[22px] h-[22px] rounded-full flex-shrink-0" style={{ background: "linear-gradient(135deg,#F8FAFC,#94A3B8)" }}>
              <div className="absolute inset-[3px] rounded-full" style={{ background: "#050506" }} />
            </div>
            <span className="text-[#F8FAFC]" style={{ fontFamily: "var(--font-dm-serif), serif", fontStyle: "italic", fontSize: "34px", letterSpacing: "-0.01em" }}>SOLO</span>
          </Link>
        ...
        <p className="text-center text-zinc-400 text-sm mt-6">
          Já tem uma conta?{" "}
          <Link href="/login" className="text-zinc-400 hover:text-zinc-200 font-medium transition-colors">Entrar</Link>
        </p>
      </div>
    </main>
  );
