    ...
    return (
      <main className="min-h-screen bg-white flex items-center justify-center px-6">
        <div className="w-full max-w-sm text-center flex flex-col gap-6">
          <div className="w-16 h-16 rounded-2xl bg-violet-50 border border-violet-200 flex items-center justify-center mx-auto">
          ...
          <div>
            <h2 className="text-2xl font-bold text-zinc-900">Verifique seu email</h2>
            <p className="text-zinc-500 text-sm mt-2 leading-relaxed">
              Enviamos um link para <span className="text-violet-500 font-medium">{form.email}</span>. Clique no link para ativar sua conta.
            </p>
          </div>
          <Link href="/login" className="text-violet-500 hover:text-violet-600 text-sm font-medium transition-colors">
            Já confirmei, entrar →
          </Link>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-white flex items-center justify-center px-6 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-400 text-sm mt-1">É grátis e leva menos de 1 minuto</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          {[
            ...
            <div key={f.id} className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-zinc-700" htmlFor={f.id}>{f.label}</label>
              <input
        ...
        <p className="text-center text-zinc-500 text-sm mt-6">
          Já tem uma conta?{" "}
          <Link href="/login" className="text-violet-500 hover:text-violet-600 font-medium transition-colors">Entrar</Link>
        </p>
      </div>
    </main>
  );
