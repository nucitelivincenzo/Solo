...
function EventoCard({ evento, inscrito, saving, onQueroIr }: {
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        ...
        {loadingPage ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {EVENTOS.map((evento) => (
              <EventoCard key={evento.id} evento={evento} inscrito={inscritos.has(evento.id)} saving={savingId === evento.id} onQueroIr={handleQueroIr} />
            ))}
          </div>
        )}
