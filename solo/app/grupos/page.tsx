...
function gerarExplicacaoGrupo(members: MemberData[], avgCompat: number): string {
  ...
  if (viable.length < 2) {
    return avgCompat > 0 ? "Grupo em formação." : "Grupo em formação — compatibilidade será calculada em breve.";
  }

  const ambientes = viable.map((m) => m.ambiente);
  const intencoes = viable.map((m) => m.intencao);
  const allSameAmbiente = ambientes.length > 0 && ambientes.every((a) => a && a === ambientes[0]);
  const allSameIntencao = intencoes.length > 0 && intencoes.every((i) => i && i === intencoes[0]);
  ...
  if (allSameIntencao && intencoes[0]) {
    return `Todos buscam ${INTENCAO_TEXT[intencoes[0] as Intencao]}.`;
  }
  if (allSameAmbiente && ambientes[0]) {
    return `Todos preferem ${AMBIENTE_LABELS[ambientes[0] as Ambiente]}.`;
  }
...
function MemberAvatar({ member }: { member: MemberData }) {
  return (
    <div className="flex flex-col items-center gap-1.5 w-14">
      <div className={`relative w-11 h-11 rounded-full bg-gradient-to-br ${avatarGradient(member.id)} flex items-center justify-center text-white text-sm font-bold shadow-md flex-shrink-0`}>
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col gap-2 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${isForming ? "bg-amber-50 border-amber-200 text-amber-700" : "bg-green-50 border-green-200 text-green-700"}`}>
                {isForming
                  ? <><span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />Formando</>
                  : <><span className="w-1.5 h-1.5 rounded-full bg-green-500" />Completo</>
                }
              </span>
            </div>
    ...
    <div className="flex flex-col items-center gap-6 py-14 px-6 text-center bg-white border border-zinc-200 rounded-2xl">
      ...
      <div className="flex flex-col gap-3 w-full max-w-xs">
        ...
        <div className="flex items-start gap-3 px-4 py-3 bg-violet-50 border border-violet-100 rounded-xl text-left">
          ...
          <p className="text-violet-700 text-xs leading-relaxed">Seu grupo aparece aqui com a compatibilidade do time</p>
        </div>
      </div>
      <Link
        href="/eventos"
...
export default function GruposPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // ── Step 3: get all members across groups ──
      ...
      nameMap.set(user.id, user.user_metadata?.name || user.email || "Você");

      try {
        const { data: names } = await supabase.rpc("get_user_names", {
          p_user_ids: allMemberIds,
        });
        names?.forEach((n: { user_id: string; display_name: string }) => {
          nameMap.set(n.user_id, n.display_name);
        });
      } catch {
        // RPC not yet created — names fall back to "Usuário"
      }

      // ── Step 6: build grupo data ──
      const result: GrupoData[] = groupsData.map((g: { id: string; event_id: string; status: string }) => {
        const memberRows = (allMembers ?? []).filter(
          (m: { group_id: string; user_id: string }) => m.group_id === g.id
        );
        ...
        const members: MemberData[] = memberRows.map((m: { group_id: string; user_id: string }) => {
          ...
          return {
            ...
            social_behavior: p?.social_behavior ?? null,
          };
        });

        const compatMembers = members.filter((m) => isCompatComplete(m));
        const avgCompat = calcGroupAvgCompat(compatMembers as CompatUser[]);
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-10 sm:py-12">
        <div className="mb-10">
          ...
          <p className="text-zinc-500 text-base max-w-md">
            Grupos formados automaticamente por afinidade social nos eventos que você quer ir.
          </p>
        </div>
        ...
        {!ready ? (
          ...
          <>
              ...
              <div className="flex items-center gap-3 mb-8 px-4 py-3.5 bg-violet-50 border border-violet-200 rounded-2xl">
                <div className="flex -space-x-2">
                  {grupos.flatMap((g) => g.members.filter((m) => !m.isMe)).slice(0, 4).map((m) => (
                    <div
                      key={`${m.id}-stack`}
                      className={`w-8 h-8 rounded-full bg-gradient-to-br ${avatarGradient(m.id)} border-2 border-white flex items-center justify-center text-white text-xs font-bold`}
                    >
                      {getInitials(m.name)}
                    </div>
                  ))}
                ...
                <p className="text-sm text-violet-700">
                  ...
                  pessoa{totalMembros !== 1 ? "s" : ""} no{grupos.length !== 1 ? "s" : ""} seu{grupos.length !== 1 ? "s" : ""}{" "}
            ...
            <div className="flex flex-col gap-5">
              {grupos.map((g) => (
                <GrupoCard key={g.group_id} grupo={g} />
              ))}
            </div>
          </>
        )}
